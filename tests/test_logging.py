...
from django_tasks_redis.backends import _elapsed_ms, task_log_fields
...
class TestElapsedMs:
    ...
    def test_returns_a_round_number_of_milliseconds(self):
        ...
        # Not == 123: less than half a millisecond of scheduling delay
        # between the two readings would push the rounding the other way.
        assert 123 <= _elapsed_ms(start) < 200

    def test_is_zero_immediately_after_the_call(self):
        import time

        assert 0 <= _elapsed_ms(time.monotonic()) < 5
...
class TestRunTaskLogging:
    ...
    def test_completed_record_has_status_and_duration(
